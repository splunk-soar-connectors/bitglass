...
try:
    ...
    underphantom = False
    from test.actions import ActionResult, BaseConnector, phantom
...
class BitglassConnector(BaseConnector):
    ...
    def _process_html_response(self, response, action_result):
        ...
        try:
            ...
            error_text = f"Cannot parse error details: {err_msg}"
    ...
    def _handle_test_connectivity(self, param):
        ...
        if phantom.is_fail(ret_val):
            ...
            try:
                ...
                ret_val, _ = RetVal(
                    action_result.set_status(phantom.APP_SUCCESS if status["last"].ok() else phantom.APP_ERROR),
    ...
    def _save_new_container(self, action_result, artifacts, key=GC_ALERT_USER_MATCH_KEY):
        container = {
            ...
            "artifacts": [
                {
                    "name": f"{u.field.title()}: {u.username}",
                    "label": "User Alert Artifact",
                    "data": u.data,
                    "source_data_identifier": f"{u.username}_{u.time}",
        ...
        if len(container["artifacts"]) > 0:
            ...
            self.debug_print(f"Save_container (with artifacts) returns, value: {res}, reason: {msg}, id: {cid}")
    ...
    def _handle_on_poll(self, param):
        ...
        self.save_progress(f"Save_container (with artifacts) returns, value: {res}, reason: {msg}, id: {cid}")
    ...
    def _get_artifact_cef(self, id):
        url = f"{self.get_phantom_base_url()}rest/artifact/{id}/"
            ...
            params = json.loads(
                "".join(
                    [
                        ...
                        f', "newgroupname": ["{newGroupName}"]' if newGroupName != "" else "",
                        "}",
                    ]
                )
            )
            ...
            if haveUserName:
                params = json.loads(
                    "".join(
                        [
                            "{",
                            '"groupname": "{}", "companyemail": [{}]'.format(groupName, ",".join(['"' + u + '"' for u in self.newUsers])),
                            "}",
                        ]
                    )
                )
            ...
            try:
                params = json.loads(
                    "".join(
                        [
                            "{",
                            '"groupname": "{}", "companyemail": [{}]'.format(groupName, ",".join(['"' + u + '"' for u in [userName]])),
                            "}",
                        ]
                    )
                )
        ...
        try:
            params = json.loads(
                "".join(
                    [
                        "{",
                        f'"companyemail": "{userName}"',
                        f', "firstname": ["{firstName}"]' if firstName != "" else "",
                        f', "lastname": "{lastName}"' if lastName != "" else "",
                        f', "secondaryemail": "{secondaryEmail}"' if secondaryEmail != "" else "",
                        f', "netbiosdomain": "{netbiosDomain}"' if netbiosDomain != "" else "",
                        f', "samaccountname": "{samAccountName}"' if samAccountName != "" else "",
                        f', "userprincipalname": "{userPrincipalName}"' if userPrincipalName != "" else "",
                        f', "objectguid": "{objectGuid}"' if objectGuid != "" else "",
                        f', "countrycode": "{countryCode}"' if countryCode != "" else "",
                        f', "mobilenumber": "{mobileNumber}"' if mobileNumber != "" else "",
                        f', "adminrole": "{adminRole}"' if adminRole != "" else "",
                        # Support just one group for now
                        f', "groupmembership": ["{groupMembership}"]' if groupMembership != "" else "",
                        "}",
                    ]
                )
            )
    ...
    def bgLoadConfig(self, conf):
        ...
        if config.get("proxies", ""):
            try:
                ...
                err_msg = self._get_error_message_from_exception(ex)
                self.debug_print(f"Bad proxy param while getting configuration params {err_msg}")
